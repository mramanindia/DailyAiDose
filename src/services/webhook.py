...
def _format_markdown_for_slack(value: str) -> str:
...
def _prepare_variables_for_body(
    ...
    summary = _format_markdown_for_webhook(str(variables["summary"]))
    if (platform or "").lower() == "slack":
        summary = _format_markdown_for_slack(summary)
...
class WebhookNotifier:
    ...
    def _render_request_components(
        ...
        body_variables = _prepare_variables_for_body(
            raw_body, variables, getattr(self.config, "platform", "generic")
        )
